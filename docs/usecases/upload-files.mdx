...
class _FileUploadWidgetState extends State<FileUploadWidget> {
  ...
  Future<void> _selectAndQueueFile() async {
    ...
    if (result != null) {
      ...
      setState(() {
        _pendingUploads.add(file.name);
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('File queued for upload: ${file.name}')),
      );
    }
  }
...
<CardGroup cols={2}>
  <Card title="Data Sync" icon="arrows-rotate" href="/usecases/data-sync">
    Synchronize data with your backend
  </Card>
  <Card title="Database Maintenance" icon="database" href="/usecases/database-maintenance">
    Clean up and optimize local data
  </Card>
</CardGroup>